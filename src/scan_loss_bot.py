...
class SimpleArbitrageBot:
    ...
    def execute_arbitrage(self, opportunity: dict):
        ...
        if self.settings.cooldown_seconds and (now - self._last_execution_ts) < float(self.settings.cooldown_seconds):
            ...
            logger.warning(f"Cooldown active ({self.settings.cooldown_seconds}s); skipping execution")
            return
    ...
    async def run_once_async(self) -> bool:
        ...
        if price_up is not None and price_down is not None:
            ...
            logger.info(
                f"无套利: UP=${price_up:.4f} ({size_up:.0f}) + DOWN=${price_down:.4f} ({size_down:.0f}) "
    ...
    async def monitor(self, interval_seconds: int = 30):
        ...
        logger.info("=" * 70)
        logger.info("🚀 尾盘扫描机器人已启动")
        logger.info("=" * 70)
        logger.info(f"市场: {self.market_slug}")
        logger.info(f"剩余时间: {self.get_time_remaining()}")
        logger.info(f"模式: {'🔸 SIMULATION' if self.settings.dry_run else '🔴 REAL TRADING'}")
        logger.info(f"成本阈值: ${self.settings.target_pair_cost:.3f}")
        logger.info(f"订单大小: {self.settings.order_size} shares")
        logger.info(f"间隔时间: {interval_seconds}s")
        logger.info("=" * 70)
        logger.info("")
        ...
        try:
            while True:
                ...
                if not self.settings.dry_run:
                    logger.info(f"已执行的交易: {self.trades_executed}")
...
async def main():
    ...
    try:
        ...
        if not ConfigValidator.validate_and_print(settings):
            print_error("配置验证失败。请修复错误并重试.")
            return
        
        print_header("🚀 终端市场扫描机器人")
        print_success("配置已加载并验证")
        ...
        def on_shutdown():
            if bot.stats_tracker:
                ...
                logger.info("📊 最终统计")
                logger.info("=" * 70)
                logger.info(f"交易总额: {stats.total_trades}")
                logger.info(f"投资总额: ${stats.total_invested:.2f}")
                logger.info(f"预期利润总额: ${stats.total_expected_profit:.2f}")
                if stats.total_actual_profit > 0:
                    logger.info(f"实际利润总额: ${stats.total_actual_profit:.2f}")
                logger.info("=" * 70)
